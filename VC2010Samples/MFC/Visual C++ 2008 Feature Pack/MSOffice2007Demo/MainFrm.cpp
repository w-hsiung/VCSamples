...
void CMainFrame::AddMainCategory()
{
	...
	std::auto_ptr<CMFCRibbonButton> apBtnSaveAs(new CMFCRibbonButton(ID_FILE_SAVE_AS, _T("Save &As"), 3, 3));
	apBtnSaveAs->AddSubItem(new CMFCRibbonLabel(_T("Save a copy of the document")));
	apBtnSaveAs->AddSubItem(new CMFCRibbonButton(ID_FILE_SAVE_AS_DOCUMENT, _T("&Word Document"), 9, 9, TRUE));
	apBtnSaveAs->AddSubItem(new CMFCRibbonButton(ID_FILE_SAVE_AS_TEMPLATE, _T("Word &Template"), 10, 10, TRUE));
	apBtnSaveAs->AddSubItem(new CMFCRibbonButton(ID_FILE_SAVE_AS_DOCUMENT_OLD, _T("Word &97-2003 Document"), 11, 11, TRUE));
	apBtnSaveAs->AddSubItem(new CMFCRibbonButton(ID_FILE_SAVE_AS_ADDINS, _T("&Find add-ins for other file formats"), 12, 12, TRUE));
	apBtnSaveAs->AddSubItem(new CMFCRibbonButton(ID_FILE_SAVE_AS_OTHER, _T("&Other Formats"), 3, 3, TRUE));
	pMainPanel->Add(apBtnSaveAs.release());
	...
	apBtnPrint->AddSubItem(new CMFCRibbonButton(ID_FILE_PRINT_DIRECT, _T("&Quick Print"), 13, 13, TRUE));
	apBtnPrint->AddSubItem(new CMFCRibbonButton(ID_FILE_PRINT_PREVIEW, _T("Print Pre&view"), 14, 14, TRUE));
	pMainPanel->Add(apBtnPrint.release());

	std::auto_ptr<CMFCRibbonButton> apBtnPrepare(new CMFCRibbonButton(ID_FILE_PREPARE, _T("Pr&epare"), 5, 5));
	apBtnPrepare->AddSubItem(new CMFCRibbonLabel(_T("Prepare the document for distribution")));
	apBtnPrepare->AddSubItem(new CMFCRibbonButton(ID_FILE_PREPARE_PROPERTIES, _T("&Properties"), 15, 15, TRUE));
	apBtnPrepare->AddSubItem(new CMFCRibbonButton(ID_FILE_PREPARE_INSPECT, _T("&Inspect Document"), 16, 16, TRUE));
	apBtnPrepare->AddSubItem(new CMFCRibbonButton(ID_FILE_PREPARE_ENCRYPT, _T("&Encrypt Document"), 17, 17, TRUE));
	apBtnPrepare->AddSubItem(new CMFCRibbonButton(ID_FILE_PREPARE_ADD_SIGNATURE, _T("Add a Digital &Signature"), 18, 18, TRUE));
	apBtnPrepare->AddSubItem(new CMFCRibbonButton(ID_FILE_PREPARE_MARK_AS_FINAL, _T("Mark as &Final"), 19, 19, TRUE));
	apBtnPrepare->SetDefaultCommand(FALSE);
	pMainPanel->Add(apBtnPrepare.release());

	std::auto_ptr<CMFCRibbonButton> apBtnSend(new CMFCRibbonButton(ID_FILE_SEND, _T("Sen&d"), 6, 6));
	apBtnSend->AddSubItem(new CMFCRibbonLabel(_T("Send a copy of the document to other people")));
	apBtnSend->AddSubItem(new CMFCRibbonButton(ID_FILE_SEND_EMAIL, _T("&E-mail"), 21, 21, TRUE));
	apBtnSend->AddSubItem(new CMFCRibbonButton(ID_FILE_SEND_FAX, _T("Internet Fa&x"), 22, 22, TRUE));
	apBtnSend->SetDefaultCommand(FALSE);
	pMainPanel->Add(apBtnSend.release());

	std::auto_ptr<CMFCRibbonButton> apBtnPublish(new CMFCRibbonButton(ID_FILE_PUBLISH, _T("P&ublish"), 7, 7));
	apBtnPublish->AddSubItem(new CMFCRibbonLabel(_T("Distribute the document to other people")));
	apBtnPublish->AddSubItem(new CMFCRibbonButton(ID_FILE_PUBLISH_BLOG, _T("&Blog"), 23, 23, TRUE));
	apBtnPublish->AddSubItem(new CMFCRibbonButton(ID_FILE_PUBLISH_DOCMANAGE_SERVER, _T("&Document Management Server"), 24, 24, TRUE));
	apBtnPublish->AddSubItem(new CMFCRibbonButton(ID_FILE_PUBLISH_CREATE_DOCWORKSPACE, _T("&Create Document Workspace"), 25, 25, TRUE));
	apBtnPublish->SetDefaultCommand(FALSE);
	pMainPanel->Add(apBtnPublish.release());
...
void CMainFrame::AddTab_Home()
{
	...
	std::auto_ptr<CMFCRibbonButtonsGroup> apFontGroup(new CMFCRibbonButtonsGroup);
	...
	pPanelFont->Add(apFontGroup.release());
	...
	std::auto_ptr<CMFCRibbonGallery> apStyleBtn(new CMFCRibbonGallery(ID_WRITE_QUICKSTYLES, _T("Quick Styles"), 9, 1, IDB_STYLES, 64));
	apStyleBtn->AddSubItem(new CMFCRibbonButton(ID_WRITE_SAVESTYLE, _T("Save Selection as a New &Quick Style..."), -1, -1));
	apStyleBtn->AddSubItem(new CMFCRibbonButton(ID_WRITE_CLEARFORMATTING, _T("&Clear Formatting"), 8, -1));
	apStyleBtn->AddSubItem(new CMFCRibbonButton(ID_WRITE_APPLYSTYLES, _T("&Apply Styles"), 9, -1));
	for (int i = 0; i < m_arStyles.GetSize(); i++)
	{
		apStyleBtn->SetItemToolTip(i, m_arStyles [i]);
	}

	apStyleBtn->EnableMenuResize();
	pPanelStyle->Add(apStyleBtn.release());

	std::auto_ptr<CMFCRibbonButton> apStyleChangeBtn(new CMFCRibbonButton(ID_WRITE_CHANGESTYLES, _T("Change Styles"), 9, 2));

	std::auto_ptr<CRibbonListButton> apStyleSetPalette(new CRibbonListButton(ID_STYLECHANGE_STYLESET, _T("St&yle Set"), 32, -1));
	apStyleSetPalette->AddGroup(_T(""), 0, 0, m_arStyleSets);
	apStyleSetPalette->AddSubItem(new CMFCRibbonButton(0, _T("&Reset to Quick Styles from Template")));
	apStyleSetPalette->EnableMenuResize(TRUE, TRUE); // Vertical only
	apStyleSetPalette->EnableMenuSideBar();

	apStyleChangeBtn->AddSubItem(apStyleSetPalette.release());

	std::auto_ptr<CRibbonListButton> apColorsPalette(new CRibbonListButton(ID_STYLECHANGE_COLORS, _T("&Colors"), 33, -1));
	apColorsPalette->AddGroup(_T("Built-In"), IDB_THEME_COLORS, 103, m_arColorThemes);
	apColorsPalette->AddSubItem(new CMFCRibbonButton(ID_PAGELAYOUT_COLORS_CREATENEWTHEMECOLORS, _T("&Create New Theme Colors...")));
	apColorsPalette->EnableMenuResize(TRUE, TRUE); // Vertical only

	apStyleChangeBtn->AddSubItem(apColorsPalette.release());

	std::auto_ptr<CRibbonListFontButton> apFontsPalette(new CRibbonListFontButton(ID_STYLECHANGE_FONTS, _T("&Fonts"), 34, -1));
	apFontsPalette->AddGroup(_T("Built-In"), IDB_THEME_FONTS, 43, m_arFonts);
	apFontsPalette->AddSubItem(new CMFCRibbonButton(ID_PAGELAYOUT_FONTS_CREATENEWTHEMEFONTS, _T("&Create New Theme Fonts...")));
	apFontsPalette->EnableMenuResize(TRUE, TRUE); // Vertical only

	apStyleChangeBtn->AddSubItem(apFontsPalette.release());

	apStyleChangeBtn->SetAlwaysLargeImage();
	apStyleChangeBtn->SetDefaultCommand(FALSE);

	pPanelStyle->Add(apStyleChangeBtn.release());
...
void CMainFrame::AddTab_Insert()
{
	...
	std::auto_ptr<CMFCRibbonGallery> apInsertShapesBtn(new CMFCRibbonGallery(ID_INSERT_SHAPES, _T("Shapes"), 6, 6));
	apInsertShapesBtn->SetButtonMode();
	apInsertShapesBtn->SetIconsInRow(12);

	apInsertShapesBtn->AddGroup(_T("Recently Used Shapes"), IDB_SHAPE1, 20);
	apInsertShapesBtn->AddGroup(_T("Lines"), IDB_SHAPE2, 20);
	apInsertShapesBtn->AddGroup(_T("Basic Shapes"), IDB_SHAPE3, 20);
	apInsertShapesBtn->AddGroup(_T("Block Arrows"), IDB_SHAPE4, 20);
	apInsertShapesBtn->AddGroup(_T("Flowchart"), IDB_SHAPE5, 20);
	apInsertShapesBtn->AddGroup(_T("Callouts"), IDB_SHAPE6, 20);
	apInsertShapesBtn->AddGroup(_T("Stars and Banners"), IDB_SHAPE7, 20);

	apInsertShapesBtn->AddSubItem(new CMFCRibbonButton(ID_INSERT_SHAPESNEW, _T("&New Drawing Canvas"), 29, -1));

	pPanelIllustrations->Add(apInsertShapesBtn.release());
...
void CMainFrame::AddTab_PageLayout()
{
	...
	std::auto_ptr<CRibbonListButton> apListBtn1(new CRibbonListButton(ID_PAGELAYOUT_THEMES, _T("Themes\nth"), 23, 0, FALSE));
	apListBtn1->AddGroup(_T("Built-In"), IDB_THEME_THEMES, 64, m_arThemes);
	apListBtn1->AddSubItem(new CMFCRibbonButton(ID_PAGELAYOUT_THEMES_RESETFROMTEMPLATE, _T("&Reset to Theme from Template")));
	apListBtn1->AddSubItem(new CMFCRibbonButton(ID_PAGELAYOUT_THEMES_MORETHEMES, _T("More Themes on Microsoft &Office Online...")));
	apListBtn1->AddSubItem(new CMFCRibbonButton(ID_PAGELAYOUT_THEMES_BROWSEFORTHEMES, _T("&Browse for Themes..."), 24));
	apListBtn1->AddSubItem(new CMFCRibbonButton(ID_PAGELAYOUT_THEMES_SAVECURRENTTHEME, _T("&Save Current Theme..."), 25));
	apListBtn1->SetIconsInRow(4);
	apListBtn1->EnableMenuResize();
	pPanelThemes->Add(apListBtn1.release());

	std::auto_ptr<CRibbonListButton> apListBtn2(new CRibbonListButton(ID_PAGELAYOUT_COLORS, _T("Colors\ntc"), 0, -1));
	apListBtn2->AddGroup(_T("Built-In"), IDB_THEME_COLORS, 103, m_arColorThemes);
	apListBtn2->AddSubItem(new CMFCRibbonButton(ID_PAGELAYOUT_COLORS_CREATENEWTHEMECOLORS, _T("&Create New Theme Colors...")));
	apListBtn2->EnableMenuResize(TRUE, TRUE); // Vertical only
	pPanelThemes->Add(apListBtn2.release());

	std::auto_ptr<CRibbonListFontButton> apFontsBtn(new CRibbonListFontButton(ID_PAGELAYOUT_FONTS, _T("Fonts\ntf"), 1, -1));
	apFontsBtn->AddGroup(_T("Built-In"), IDB_THEME_FONTS, 43, m_arFonts);
	apFontsBtn->AddSubItem(new CMFCRibbonButton(ID_PAGELAYOUT_FONTS_CREATENEWTHEMEFONTS, _T("&Create New Theme Fonts...")));
	apFontsBtn->EnableMenuResize(TRUE, TRUE); // Vertical only
	pPanelThemes->Add(apFontsBtn.release());

	std::auto_ptr<CRibbonListButton> apListBtn3(new CRibbonListButton(ID_PAGELAYOUT_EFFECTS, _T("Effects\nte"), 2, -1, FALSE));
	apListBtn3->AddGroup(_T("Built-In"), IDB_THEME_EFFECTS, 64, m_arThemes);
	apListBtn3->SetIconsInRow(4);
	apListBtn3->EnableMenuResize();
	pPanelThemes->Add(apListBtn3.release());
	...
	CMFCRibbonNumEdit* pEditIndentLeft = new CMFCRibbonNumEdit(ID_PAGELAYOUT_INDENT_LEFT, 72, _T("Left:\nil"), 13);
	pEditIndentLeft->EnableSpinButtons(0, 1000);
	pEditIndentLeft->SetEditText(_T("0"));
	pPanelParagraph->Add(pEditIndentLeft);

	CMFCRibbonNumEdit* pEditIndentRight = new CMFCRibbonNumEdit(ID_PAGELAYOUT_INDENT_RIGHT, 72, _T("Right:\nir"), 14);
	...
	CMFCRibbonNumEdit* pEditSpaceBefore = new CMFCRibbonNumEdit(ID_PAGELAYOUT_SPACE_BEFORE, 72, _T("Before:\nsb"), 15);
	pEditSpaceBefore->EnableSpinButtons(0, 100);
	pEditSpaceBefore->SetEditText(_T("0"));
	pPanelParagraph->Add(pEditSpaceBefore);

	CMFCRibbonNumEdit* pEditSpaceAfter = new CMFCRibbonNumEdit(ID_PAGELAYOUT_SPACE_AFTER, 72, _T("After:\nsa"), 16);
...
void CMainFrame::AddTab_Mailings()
{
	...
	pPanelWriteFields->Add(new CMFCRibbonButton(ID_MAILINGS_GREETINGLINE, _T("Greeting Line\ng"), 7, 7));
	...
	std::auto_ptr<CMFCRibbonButtonsGroup> apButtonsListRecords(new CMFCRibbonButtonsGroup);

	apButtonsListRecords->AddButton(new CMFCRibbonButton(ID_MAILINGS_FIRSTRECORD, _T("\nq"), 16));
	apButtonsListRecords->AddButton(new CMFCRibbonButton(ID_MAILINGS_PREVIOUSRECORD, _T("\nm"), 17));

	CMFCRibbonEdit* pEdit = new CMFCRibbonEdit(ID_MAILINGS_GOTORECORD, 40);
	pEdit->SetKeys(_T("w"));
	apButtonsListRecords->AddButton(pEdit);

	apButtonsListRecords->AddButton(new CMFCRibbonButton(ID_MAILINGS_NEXTRECORD, _T("\nx"), 18));
	apButtonsListRecords->AddButton(new CMFCRibbonButton(ID_MAILINGS_LASTRECORD, _T("\nv"), 19));

	pPanelReview->Add(apButtonsListRecords.release());
...
void CMainFrame::AddContextTab_Chart()
{
	...
	pPanelLayout_Axes->SetKeys(_T("zx"));
	...
	pPanelFormat_Shape->SetKeys(_T("zy"));
...
void CMainFrame::ShowOptions(int nPage)
{
	...
	std::auto_ptr<CDemoOptionsPage> apPageOptions(new CDemoOptionsPage);
	std::auto_ptr<CResourcePage> apPageRes(new CResourcePage);

	// Create property sheet:
	std::tr1::shared_ptr<CDemoOptionsPropSheet> spPropSheet(new CDemoOptionsPropSheet(this, nPage));
